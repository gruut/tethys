...
KBucket::KBucket(Node node, unsigned int depth, std::size_t ksize) : m_my_node(std::move(node)), m_depth(depth), m_ksize(ksize) {
  touchLastUpdated();
}
...
void KBucket::removeNode(Node const &node) {
  ...
  auto node_found =
      std::find_if(begin(), end(), [&node](Node const &bucket_node) { return (bucket_node.getIdHash() == node.getIdHash()); });
  if (node_found != end()) {
    removeNode(node_found);
  } else {

    auto replacement_found = std::find_if(m_replacement_nodes.begin(), m_replacement_nodes.end(), [&node](Node const &replacement_node) {
...
std::vector<Node> KBucket::selectAliveNodes(bool force_to_select_all) {
  ...
  std::copy_if(m_nodes.begin(), m_nodes.end(), std::back_inserter(alive_nodes),
               [force_to_select_all](auto &n) { return force_to_select_all || n.isAlive(); });
  return alive_nodes;
}
...
std::chrono::seconds KBucket::timeSinceLastUpdated() const {
  return std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - m_last_updated);
}

bool KBucket::hasReplacements() const {
  return !m_replacement_nodes.empty();
}
