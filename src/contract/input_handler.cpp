...
namespace tethys::tsce {
  bool tsce::InputHandler::parseInput(nlohmann::json &input_json, tinyxml2::XMLElement *input_node,
    ...
    for (int i = 0; i < num_input; ++i) {
      ...
      for (auto &each_input : input_json[i]) {
        for (auto &each_item : each_input.items()) {
          ...
          if (!is_valid_key ||
              !isValidValue(input_value, input_options[opt_idx].type, input_options[opt_idx].validation))
            continue;
