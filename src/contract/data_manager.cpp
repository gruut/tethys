...
int64_t DataManager::getUserKeyCurrency(std::string_view user_id_) {
  ...
  if (!result.empty() && result[0].name == m_keyc_name) {
    keyc_amount = mt::str2num<uint64_t>(result[0].string_value);
  }

  return keyc_amount;
}

std::optional<UserScopeRecord> DataManager::findUserScopeTableByName(std::string_view user_id, std::string_view var_name) {
...
std::optional<UserScopeRecord> DataManager::findUserScopeTableByPid(std::string_view user_id, std::string_view var_name,
                                                                    std::string_view pid) {
  ...
  if (!result_name.empty() && !result_data.empty()) {
    ...
    for (auto &each_row : result_data) {
      for (int i = 0; i < each_row.size(); ++i) { // last row will be selected
        if (each_row[i].is_boolean()) {
          bool value = each_row[i].get<bool>();
...
std::vector<DataAttribute> DataManager::queryIfUserAttrAndParseData(nlohmann::json &query, const std::string &user_id) {
...
vector<DataAttribute> DataManager::queryIfUserScopeAndParseData(nlohmann::json &query, std::string_view user_id, string_view pid,
                                                                bool get_val) {
...
std::vector<DataAttribute> DataManager::queryIfContractScopeAndParseData(nlohmann::json &query, const std::string &cid) {
