...
namespace tethys::tsce {
  ...
  bool tsce::EndorserHandler::evalue(tinyxml2::XMLElement *doc_node, DataManager &data_manager) {
    ...
    switch (base_condition_type) {
      ...
      case SecondaryConditionType::NIF: {
        EvalRuleType base_eval_rule = getEvalRule(mt::c2s(doc_node->Attribute("eval-rule"))).value_or(
                EvalRuleType::AND);
